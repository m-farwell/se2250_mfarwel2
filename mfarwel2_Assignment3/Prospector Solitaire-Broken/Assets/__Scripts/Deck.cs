...
public class Deck : MonoBehaviour {
	...
	public void MakeCards() {
		...
		foreach (string s in letters) {
            // Error 3: for loop should only go up to 13 as 13 cards in each suit.
			for (int i=0; i<13; i++) {
				cardNames.Add(s+(i+1));
			}
		}
		...
		for (int i=0; i<cardNames.Count; i++) {
            ...
            // Error 5: Cards order is chronological. Call Shuffle function to randomly order the list
            Shuffle(cards);
		}
	}

    // Error 5 cntd. Shuffle function implementation
